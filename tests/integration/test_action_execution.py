...
async def test_execute_pending_request_denied(async_client, db_session, sample_investigation):
    ...
    # Try execute
    exec_response = await async_client.post(f"/api/v1/action-requests/{request_id}/execute", json={"idempotency_key": "test_1"})
...
async def test_execute_approved_request_success(async_client, db_session, sample_investigation):
    ...
    # Approve request
    approve_resp = await async_client.post(f"/api/v1/action-requests/{request_id}/approve", json={"actor": "test"})
    assert approve_resp.status_code == status.HTTP_200_OK
    
    # Execute request
    exec_response = await async_client.post(f"/api/v1/action-requests/{request_id}/execute", json={"idempotency_key": "test_success"})
...
async def test_execute_simulator_failure(async_client, db_session, sample_investigation):
    ...
    request_id = response.json()["id"]
    await async_client.post(f"/api/v1/action-requests/{request_id}/approve", json={"actor": "test"})
    
    # Execute request with fail key
    exec_response = await async_client.post(f"/api/v1/action-requests/{request_id}/execute", json={"idempotency_key": "test_simulate_fail"})
    assert exec_response.status_code == status.HTTP_200_OK
...
async def test_execute_simulator_unknown(async_client, db_session, sample_investigation):
    ...
    exec_response = await async_client.post(f"/api/v1/action-requests/{request_id}/execute", json={"idempotency_key": "test_simulate_unknown"})
    assert exec_response.status_code == status.HTTP_200_OK
...
async def test_execution_concurrency_and_idempotency(async_client, db_session, sample_investigation):
    ...
    executions_resp = await async_client.get(f"/api/v1/action-requests/{request_id}/executions")
